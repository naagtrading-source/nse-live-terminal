...
def process_and_render_view(is_stock_view, dropdown_options):
    ...
    with c2:
        selected_expiry = st.selectbox("Select Expiry Series", get_expiry_dates_local(), key=f"ex_{is_stock_view}")
    ...
    if 'global_history' in st.session_state and st.session_state.global_history:
        ...
        for item in h_list:
            if item.get('IsStock', False) != is_stock_view or item.get('Expiry') != selected_expiry:
                continue
