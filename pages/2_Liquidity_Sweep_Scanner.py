...
st.set_page_config(page_title="Institutional Liquidity Sweep Scanner", layout="wide", page_icon="💥")
...
st.caption("Advanced Order Flow Trapping Index | Live Cross-Asset Multi-Grid Terminal")
...
def load_live_spikes_from_db():
    ...
    try:
        ...
        # ORDER BY id DESC ensures the fresh anomalies always show at the top
...
def render_terminal_log_block(asset_filter, df_source):
    if df_source.empty:
        st.markdown("<p style='color:#666;font-size:0.85rem;padding-left:10px;'>📡 Awaiting live block orders or Colab script data generation ticks...</p>", unsafe_allow_html=True)
        return
        
    # Filter for the chosen asset
    f_df = df_source[df_source['asset'].str.upper() == asset_filter.upper()].copy()
    
    if f_df.empty:
        st.markdown(f"<p style='color:#666;font-size:0.85rem;padding-left:10px;'>Scanning live {asset_filter} order flow books...</p>", unsafe_allow_html=True)
        return
...
def render_liquidity_grid():
    all_df = load_live_spikes_from_db()
    
    st.markdown("<div class='section-header'>⚡ LIQUIDITY ABSORPTION TARGET MAPS</div>", unsafe_allow_html=True)
    
    idx_col1, idx_col2 = st.columns(2)
    with idx_col1:
        st.markdown("<div class='asset-title-banner'>💥 NIFTY LIQUIDITY SWEEPS</div>", unsafe_allow_html=True)
        render_terminal_log_block("NIFTY", all_df)
    with idx_col2:
        st.markdown("<div class='asset-title-banner'>💥 BANKNIFTY LIQUIDITY SWEEPS</div>", unsafe_allow_html=True)
        render_terminal_log_block("BANKNIFTY", all_df)

render_liquidity_grid()
